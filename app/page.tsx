...
const RESULTS: Record<ResultType, ResultData> = {
  gas: {
    ...
    improvements: [
      ...
      {title:"食後15〜30分のウォーキングで腸を動かす", detail:"食後すぐ座り続けると食べ物が腸内に滞留してガスが増えます。軽いウォーキングは腸の蠕動運動を促し、ガスを自然に排出させる最も効果的な方法の一つです。激しい運動は逆効果になる場合があるため、食後の散歩レベルの軽い運動から始めましょう。"},
  ...
  con: {
    ...
    improvements: [
      ...
      {title:"座り時間を1時間に1回必ず中断する", detail:"長時間の座位は腸の蠕動運動を著しく低下させます。仕事中でも1時間に1回は立ち上がり、5分程度歩くだけで腸の動きが改善します。食後の10分ウォーキングも腸の蠕動運動を強力に促進します。エレベーターを使わず階段を使うなど、日常の中に「腸を動かす機会」を増やしましょう。"},
    ],
  },
  ...
  mix: {
    ...
    weekPlan: [
      ...
      "日：30分の軽い有酸素運動（腸内フローラの多様性改善に効果的）",
    ],
    ...
    improvements: [
      ...
      {title:"週3回以上の有酸素運動で腸内フローラの多様性を高める", detail:"運動は腸内フローラの多様性を直接高める効果があることが近年の研究で明らかになっています。特に週3回以上の有酸素運動（ウォーキング・ジョギング・自転車など）が腸内細菌の種類を増やすことが示されています。激しい運動でなくても30分のウォーキングを週3回続けるだけで効果があります。"},
    ],
  },
};
